            ...
            msg.on('body', async (stream, info) => {
              ...
              if (parsed.attachments.length > 0) {
                for (let att of parsed.attachments) {
                  if (self.attachments) {
                    const getFileName = (filename, self) => `${self.attachmentOptions.directory}${
                      Buffer.from(Math.random().toString()).toString("base64").substr(10, 5)
                      +
                      Date.now()
                    }${
                      isNull(filename) ? '' : filename.replace(/[\n\r\t\s]+/g, '')
                    }`
                    let filename = getFileName(att.filename, self)
                    while(fs.existsSync(filename)){
                      filename = getFileName(att.filename, self)
                    }
                    try {
                      await fs.promises.writeFile(`${/*self.attachmentOptions.directory}${*/filename/*att.filename*/}`, att.content, (error) => {
                        // self.emit('error', error);
                        // console.log('error charge local file ', att.filename, filename);
                        
                      });
                      self.emit('attachment', att, filename, seqno);
                    } catch (e) {
                      self.emit('attachment', att, null, seqno);
                      // console.log('Error during writeFileFromEmail :>> ', e);
                    }
